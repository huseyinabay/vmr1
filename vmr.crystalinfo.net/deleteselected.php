...
if (!isset($_SESSION['username']) || $_SESSION['usertype'] != 'admin') { // if session variable "username" does not exist.
    ...
    foreach ($_POST['checklist'] as $singleVar) {
        ...
        $result = mysqli_query($db->getConnection(), $SQL) or die(mysqli_error());
        ...
        if ($checkuser > 0) {
            ...
            mysqli_query($db->getConnection(), "DELETE FROM $tablename WHERE id='$singleVar'") or die(mysqli_error());

            $i++;
        }

    }
    header("location: $return?cmsg=$i Kayıtlar Başarı ile Silinmiştir!");
