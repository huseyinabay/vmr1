		...
		foreach($fields as $field)
		{
			...
			else
			{
				...
				if(is_string($value))
				{
					...
					$value = strip_tags($value); // HTML tag'leri temizler
$value = htmlspecialchars($value, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); // XSS koruması
	...
	public function get_readable_errors($convert_to_string = false, $field_class="field", $error_class="error-message")
	{
		...
		foreach($this->errors as $e) {
			...
			if ($field =='Status1'){$field ="Giriş Türü";}
			if ($field =='Billnumber'){$field ="İrsaliye Numarası";}
			if ($field =='Item'){$field ="Ürün Seçimi";}
			
			$param = $e['param'];
			
			switch($e['rule']) {
				case 'validate_required':
					$resp[] = "<span class=\"$field_class\">$field</span> Geçersiz. Lütfen kontrol edin !";
					break;
				case 'validate_valid_email':
					$resp[] = "<span class=\"$field_class\">$field</span> geçerli bir e-mail adresi girmelisiniz";
					break;
				case 'validate_max_len':
					if($param == 1) {
						$resp[] = "<span class=\"$field_class\">$field</span> en fazla $param karakterden oluşmak zorundadır !";
					} else {
						$resp[] = "<span class=\"$field_class\">$field</span> en fazla $param karakterden oluşmak zorundadır !";
					}
					break;
				case 'validate_min_len':
					if($param == 1) {
						$resp[] = "<span class=\"$field_class\">$field</span> en az $param karakterden oluşmak zorundadır !";
					} else {
						$resp[] = "<span class=\"$field_class\">$field</span> en az $param karakterden oluşmak zorundadır !";
					}
					break;
