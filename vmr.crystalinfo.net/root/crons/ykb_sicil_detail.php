  ...
  while($row = mysqli_fetch_object($rs)){//Bütün siteler taranmalı Site döngüsü start
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapılacakır. Mail atılacak mı start
      if($row->MONTHLY_MAILING_DAY == date("d") || $force==1){//Mail günü geldi mi start
        ...
        <center>  
        <br><br>
        Geçen aya ait telefon görüşme detayınız ektedir. <br>Bilgi için başvuru adresi : teknik@kardemir.com.tr<br>
        $stnotes
        <table width=\"65%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                <TD width=\"100%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylık Dahili Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
              </TR>
            </TABLE>
            </td>
          </tr>
        ...
        while($rwx = mysqli_fetch_object($rslt)){//Sitedeki dahililer döngüsü start
//          $orig_dn = ;
          ...
          $orig_dn ="";
          $descr = "";
          while($rwxe = mysqli_fetch_object($rslte)){
          ...
          $DATA = "";
          if ($email!="" && $orig_dn!=""){//Mail adresi yoksa göndermenin bir anlamı yok Email var mı start
            ...
            while($row_orig = mysqli_fetch_object($result_orig)){//Rapor içeriği start
              if($row_orig->ORIG_DN != ""){//Dahili alanı dolumu start
                ...
                if($acc_code!=$row_orig->ACCESS_CODE || $r_orig != $row_orig->ORIG_DN){
                  if($r_orig!="---"){
                    $DATA  .= "<tr >
                      <td colspan=5 class=\"rep_table_header\" width=\"5%\">$r_orig Toplamı</td>
                ...
                if($acc_code!=$row_orig->ACCESS_CODE){
                  ...
                  $DATA  .= " <td class=\"rep_td\" align=\"Left\" colspan=6><b>".$acc_code_arr[$row_orig->ACCESS_CODE]." (ERİŞİM KODU : $row_orig->ACCESS_CODE)</b></td></tr>\n";
                }
                if($acc_code!=$row_orig->ACCESS_CODE || $r_orig != $row_orig->ORIG_DN){
                  $DATA  .= " <tr  BGCOLOR=\"FFFFFF\">\n<td class=\"rep_td\" align=\"Left\">&nbsp;</td>";
                  $DATA  .= " <td class=\"rep_td\" align=\"Left\" colspan=5><b>DAHİLİ NO : ".$row_orig->ORIG_DN."</b></td></tr>\n";
                  $org_total=0;
                }
