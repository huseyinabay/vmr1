  ...
  while($row = mysqli_fetch_object($rs)){//Bütün siteler taranmalı Site döngüsü start
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapılacakır. Mail atılacak mı start
      if($row->MONTHLY_MAILING_DAY == date("d") || $force==1){//Mail günü geldi mi start
        $report_name = "Aylık Top 100 Görüşme Raporu";
        $DATA_HEADER ="
        <html>
        <head>
          ...
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                ...
                <TD width=\"50%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylık Top 100 Görüşme Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
          ...
          if ($ADMIN_EMAIL<>"" || $SITE_ADMIN<>""){//Mail adresi yoksa göndermenin bir anlamı yok Email var mı start
            ...
            if($DATA !=""){
              ...
              if($prnscreen==1){
                ...
                if(strlen($sendadres)){
    			  mail_send($sendadres,"Top 100  Görüşme Raporu -- $SITE_NAME",$DATA);
                }else{
			      mail_send($SITE_ADMIN,"Top 100  Görüşme Raporu -- $SITE_NAME",$DATA);
			      mail_send($ADMIN_EMAIL,"Top 100  Görüşme Raporu -- $SITE_NAME",$DATA);
                }
              }
