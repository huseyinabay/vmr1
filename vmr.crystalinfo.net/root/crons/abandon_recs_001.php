...
//print_r ($result); die;
  ...
  while($row = mysqli_fetch_object($result)){
    ...
    $sql1 = "SELECT CDR_MAIN_INB.SITE_ID, EXTENTIONS.SITE_ID, CDR_ID, EXTENTIONS.EMAIL, TIME_STAMP, CHG_INFO, CLID, TER_DN, 
				EXTENTIONS.DESCRIPTION AS DES, 
				CONCAT(PHONES.CITY_CODE,PHONES.PHONE_NUMBER) AS P_CLID,
				CONCAT(CONTACTS.NAME, ' ', CONTACTS.SURNAME) AS ARAYAN_ADI,
				E.DESCRIPTION as EDES 
             FROM CDR_MAIN_INB
			 ...
			 LEFT JOIN PHONES ON CONCAT(PHONES.CITY_CODE,PHONES.PHONE_NUMBER) = SUBSTRING(CDR_MAIN_INB.CLID,3,11)
             LEFT JOIN CONTACTS ON CONTACTS.CONTACT_ID = PHONES.CONTACT_ID
...
//ECHO $sql1; die;			 
    ...
    while($row1 = mysqli_fetch_object($rs1)){
      ...
      $DATA = "<b> $row1->CLID - $row1->ARAYAN_ADI</b> numaralı telefon <b> $row1->TIME_STAMP </b>  tarihinde sizi( Dahili :<b> $row1->TER_DN   $row1->DES</b>) aradı, ulaşamadı. çalma süresi - $row1->CHG_INFO <BR><BR><BR>";
      $DATA .= "You( Extention :<b> $row1->TER_DN  $row1->DES</b>) called by <b>$row1->CLID - $row1->ARAYAN_ADI</b> at <b> $row1->TIME_STAMP </b> but the number couldn't reach. Ring time - $row1->CHG_INFO <BR> ";
      $sbjct = "Cyrstallinfo- $row1->CLID - $row1->ARAYAN_ADI numaralı telefon $row1->TIME_STAMP tarihinde sizi( Dahili : $row1->TER_DN $row1->DES) aradı, ulaşamadı";
