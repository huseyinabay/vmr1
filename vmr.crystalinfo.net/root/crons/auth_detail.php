  ...
  while($row = mysqli_fetch_object($rs)){//Bütün siteler taranmalı Site döngüsü start
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapılacakır. Mail atılacak mı start
      if($row->MONTHLY_MAILING_DAY == date("d") || $force==1){//Mail günü geldi mi start
        $report_name = "Aylık Şifreli Görüşme Raporu";
        $DATA_HEADER ="
        <html>
        <head>
          ...
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                ...
                <TD width=\"50%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylık Şifre Görüşme Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
        ...
        while($rwx = mysqli_fetch_object($rslt)){//Sitedeki auth_code döngüsü start
          ...
          if ($email<>""){//Mail adresi yoksa göndermenin bir anlamı yok Email var mı start
            ...
            $my_pr=0;
            while($row_auth = mysqli_fetch_object($result_auth)){//Rapor içeriği start
              if($row_auth->AUTH_ID != ""){//Auth alanı dolumu start
            ...
            if($DATA !=""){
			  ...
			  mail_send($email,"Şifre Raporu -- $auth_code.",$DATA);
