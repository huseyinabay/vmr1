...
$local_root = "/home/cisco/DATA/";  //BUFFER IN DOSYALARI KOYDUĞU YER
$list=Array(); 
...
while($list[$i]) { 
          ...
          if (file_exists($local_root.$file_name)) {
                ...
                @mkdir("/usr/local/sigma/crystal/data/".date("m")."_".date("Y")."/");
                    ...
                    $handle2= fopen("/usr/local/sigma/crystal/data/".date("m")."_".date("Y")."/3cisco_raw_".date("m")."_".date("Y").".dat","a+");
                    if ($handle) {
                     $mysayac=1;
        while (!feof($handle)) {
...
$buffernew = $bufarray[2].str_repeat(" ", 10- strlen($bufarray[2]));}                                                //Sıra No - globalCallID_callId
else{
$buffernew ="callId    ";
}
...
echo gmdate('H:i:s', $your_time_in_seconds);
			 ...
			 if(strlen($buffer)>2){
                                               ...
                                               //  $sqlQuery =  "\n INSERT INTO SEMI_FINISHED(LINE1_ID, LINE1, DATE_TIME, YEAR, MONTH, DAY, SITE_ID, DONE) VALUES (".$mysayac.", '".$buffer."',CURDATE(), YEAR(CURDATE()),MONTH(CURDATE()), DAYOFMONTH(CURDATE()), 3, '0')"; 
                                               //  $mysayac=$mysayac+1;
...
function read_dir($dir) {
     if ($handle = opendir($dir)) {
          ...
          while (false !== ($file = readdir($handle))) {
              ...
              if(substr($file,0, 9)=="cdr_METRO") $array[] = $file; // Halil  - ana dosyayı silmemesi icin yapıldı 
          }
          closedir($handle);
     }
     return $array;
