<?php   
  ...
  $arr_contact = array();
  $arr_ext = array();
  if($siteid!=""){$sitecrt = " WHERE SITE_ID=".$siteid;}
  $sql_str = "SELECT * FROM SITES ".$sitecrt;
  ...
  while($row = mysql_fetch_object($rs)){//Bütün siteler taranmalý Site döngüsü start
    ...
    $max_acc_duration = $row->MAX_ACCE_DURATION;
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapýlacakýr. Mail atýlacak mý start
          ...
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                ...
                <TD width=\"50%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylýk OZEL ARAMA  Dahili Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
	  ...
	  $kriter = "";
          ...
          if($SITE_ID==8){ //antalya bolgesi cikisi 55 yerine 56 dan yapmakta oldugu icin antalyaya ozel kriter konmustur.
	     $kriter .= $cdb->field_query($kriter,   "ACCESS_CODE"     ,  "=",  "56"); //ozel aramalarinin erisim kodudur
	  }else{
    	     $kriter .= $cdb->field_query($kriter,   "ACCESS_CODE"     ,  "=",  "66"); //ozel aramalarinin erisim kodudur
	  }
	    ...
	    $sql_str_orig  = "SELECT LTRIM(ORIG_DN) AS ORIG_DN,
	      ...
	      //mail_send('emel.karatas@axaoyak.com.tr',"Dahili Raporu -- $orig_dn.",$DATA);exit;
