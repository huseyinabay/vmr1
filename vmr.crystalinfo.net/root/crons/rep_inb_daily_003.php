...
/*******EDIT LINES 3-8*******/
	...
	//$to = "hmutlu@netser.com.tr;aabay@mniletisim.com.tr"; 
	$to = "BOGAZICI_SANTRAL_RAPOR@renault.com.tr;hmutlu@netser.com.tr;aabay@mniletisim.com.tr;mbudak@mniletisim.com.tr;okurt@netser.com.tr;herdinc@netser.com.tr";
	$subject   = "$ttype DETAY Raporu, $company Şube, Rapor Tarihi: $st0";

	$body = "Günlük Gelen Çağrı Detay Raporu. <br>
			 Şube Adı: $company <br>
			 Rapor Tarihi: $st0 <br>";
...
//create MySQL connection  
	...
	$kriter .= $cdb->field_query($kriter,   "CALL_TYPE"     ,  "=",  "2"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.      
	...
	$Connect = mysqli_connect("localhost","crinfo","SiGmA*19","MCRYSTALINFONE") or die("Couldn't connect to MySQL:<br>" . mysqli_error() . "<br>" . mysqli_errno());
	    ...
	    $strSQL ="SELECT CDR_MAIN_INB.SITE_ID,REC_TYPE,ORIG_DN,TIME_STAMP_YEAR,TIME_STAMP_MONTH,TIME_STAMP_DAY,
					TIME_STAMP_HOUR,TIME_STAMP_MN,TIME_STAMP_SN,DURATION,DIGITS,ORIG_TRUNK_MEMBER,CHG_INFO,
					...
					FROM CDR_MAIN_INB
					LEFT JOIN EXTENTIONS ON CDR_MAIN_INB.ORIG_DN = EXTENTIONS.EXT_NO AND CDR_MAIN_INB.SITE_ID = EXTENTIONS.SITE_ID
					LEFT JOIN DEPTS ON DEPTS.DEPT_ID = EXTENTIONS.DEPT_ID
					";  
		$strSQL .= " WHERE CDR_MAIN_INB.".$kriter;
		$strSQL .= " ORDER BY TIME_STAMP_HOUR, TIME_STAMP_MN";
...
<th>Hat No</th><th>Tip</th><th>Arayan Numara</th><th>Tarih</th><th>Saat</th><th>Süre</th><th>Ring</th></tr>';
...
while ($row = mysqli_fetch_assoc($result)){
	...
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.=$row['ORIG_DN']; 
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.=$row['ACIKLAMA']; 
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }  elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.=$row['DEPARTMAN'];
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.=$row['ORIG_TRUNK_MEMBER']; 
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.=$row['REC_TYPE']; 
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	$excel.= substr($row['CLID'],1);
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G"  ) {	$excel.='</td><td bgcolor="LightPink">'; }	elseif ($row['REC_TYPE'] == "t") {	$excel.='</td><td bgcolor="lightblue">'; } else {$excel.='</td><td>'; }
	...
	if ($row['REC_TYPE'] == "R" || $row['REC_TYPE'] == "G" ) {$counterA ++;}
	if ($row['REC_TYPE'] == "t") {$counterT ++;}
	if ($row['REC_TYPE'] == "I" || $row['REC_TYPE'] == "H" || $row['REC_TYPE'] == "t" ) {$tot_duration = ((int)$tot_duration) + ((int)$row['DURATION']);}
...
$body .= "Cevapsız Çağrı Sayısı: $counterA Adet <br>";
...
$body .= "<br> Not: Çağrı Tipi: R ve G Cevapsız Çağrı, I ve H Cevaplanmış Çağrı, t Transfer Edilmiş <br>";
