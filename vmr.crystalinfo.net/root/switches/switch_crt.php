...
//Site Admin değilse bu tanımı yapamamalı
   ...
   if ($act=="upd" && $id!=="" && is_numeric($id)){
       $sql_str = "SELECT * FROM SANT_SATIR_KRT WHERE ID = $id";
...
<script>function submit_form() {
    if (check_form(document.switch_crt))
        document.switch_crt.submit();
  }
    function change_santral(sw_id){
        window.location.href =  'switch_crt.php?act=new&sw_id='+sw_id
    }    
    function change_log_type(log_id){
        window.location.href = 'switch_crt.php?act=new&sw_id=<?=$sw_id?>&log_id='+log_id
    }    
</script>
  <center>
  <table width="100%" cellpadding="0" cellspacing="0">
    <form name="switch_crt" method="post" onsubmit="return check_form(this);" action="switch_crt_db.php?act=<? echo  $_GET['act'] ?>">
    <input type="hidden" name="id" value="<?=$id?>">
       ...
       <tr class="form">
           ...
           <td colspan=2>
                <input type="text" name="CRT_NAME" class="input1" size="25" value="<?=$row->NAME?>">
          </td>
    </tr>
       <tr class="form">
            <td colspan =3>
           ...
           <td class="td1_koyu">Satır No :
                ...
                <td class="td1_koyu">Değer :
                <input type="text" name="CRT_CHAR" class="input1" size="5" value="<?=$row->CRT_CHAR?>"></td>
            </tr></table>    
          </td>
    </tr>
    <tr height="15">
  ...
  <table align="right" width="100%" border="0">
    <tr>
      ...
      <td width="30%"><?if($sw_id>0 && $log_id>0){?>
            <a href="switch_fields.php?act=new&log_id=<?=$log_id?>&sw_id=<?=$sw_id?>">Alan Tanımları</a><?}?></td>
      <td align="right"><a href="switch_crt.php?act=new&log_id=<?=$log_id?>&sw_id=<?=$sw_id?>"><img border="0" src="<?=IMAGE_ROOT?>yeni_kayit.gif" style="cursor:hand;"></a></td>
    </tr>
  </table>
...
<?table_footer();
         ...
         $sql_str  = "SELECT ID, NAME, ROW_NUM, START, LENGTH, SANT_SATIR_KRT.CRT_CHAR
                        FROM SANT_SATIR_KRT
                        WHERE SWITCH_ID=".$sw_id." AND SWITCH_LOG_ID=".$log_id;
   ...
   if(mysqli_num_rows($result)>0){
       while($row = mysqli_fetch_object($result)){
          ...
          echo " <td height=\"20\">$row->ID</td> ".CR;
             ...
             echo " <td>".$row->CRT_CHAR."</td>".CR;
           echo " <td>".$row->ROW_NUM."</td>".CR;
           echo " <td>".$row->START."</td>".CR;
           echo " <td>".$row->LENGTH."</td>".CR;
           echo " <td><a HREF=\"switch_crt.php?act=upd&log_id=$log_id&id=$row->ID&sw_id=$sw_id\">Güncelle</td>".CR;
             echo "</tr>".CR;
           list_line(18);
       }
    }
...
<script language="javascript">
  ...
  form_fields[2] = Array ("LENGTH",   "Uzunluk Sayısal Olmalı.", TYP_DIGIT);
    ...
    form_fields[4] = Array ("CRT_NAME", "Alan Adını seçmeniz gerekli.", TYP_NOT_NULL);
