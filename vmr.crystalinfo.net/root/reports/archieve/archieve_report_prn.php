<?  require_once $_SERVER['DOCUMENT_ROOT']."/cgi-bin/functions.php";
    ...
    if (right_get("SITE_ADMIN")){
      ...
      echo $usr_crt  = get_ext($_SESSION["user_id"]);
    }
...
function CheckEmail (strng) {
  ...
  $sql_str="SELECT Locationid,LocationName FROM TLocation"; 
   ...
   if ($act == "src") {
      ...
      $kriter .= $cdb->field_query($kriter,   "CDR_ARCHIEVE.SITE_ID"     ,  "=",  "$SITE_ID"); //Bu mutlaka olmalı.İlgili siteyi belirliyor.
      ...
      $kriter .= $cdb->field_query($kriter,   "CDR_ARCHIEVE.ORIG_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.

      add_time_crt();//Zaman kriteri
     //Genel raporlardan dahili çağrıların detayları için gelindiğinde dahili kısmı boş olanlar için.
     if ($ORIG_DN == "-2"){
        if ($kriter == ""){
            $kriter = " CDR_ARCHIEVE.ORIG_DN = ''" ;
        }else if ($kriter <> ""){
            $kriter = $kriter. " AND CDR_ARCHIEVE.ORIG_DN = ''";
            $ORIG_DN = "";
        }
    }
    ...
    if (($TelProviderid <> '-1') && ($TelProviderid<>'')){
        $tel_provider='Yes';
        $provider_join =" LEFT JOIN TTelProvider ON CDR_ARCHIEVE.TO_PROVIDER_ID = TTelProvider.TelProviderid ";
    }
    if ($DURATION <> ''){
        $dur='Yes';
    ...
    if($in_str != ""){
        if($kriter == ""){
            $kriter .= " CDR_ARCHIEVE.ORIG_DN IN (".$in_str.")";
        }else{
            $kriter .= " AND CDR_ARCHIEVE.ORIG_DN IN (".$in_str.")";
        }
    }else{
        $orig = '';
    }
    ...
    if($in_str != ""){
        ...
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.LocationTypeid"   ,"=",        "$LocationTypeid");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.TO_PROVIDER_ID"   ,"=",        "$TelProviderid");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.LocalCode"        ,"=",        "$LocalCode");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.CountryCode"      ,"=",        "$CountryCode");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.PURE_NUMBER"      ,"LIKE",     "'%$DIGITS%'");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.AUTH_ID"          ,"=",        "'$AUTH_CODE'");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.ACCESS_CODE"      ,"=",        "'$ACCESS_CODE'");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.PRICE"            ,">=",       "$PRICE");
        $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.DURATION"         ,">",        "'$DURATION_SN'");
        if($MEMBER_NO != -1){
          $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.TER_TRUNK_MEMBER"          ,"=",       "'$MEMBER_NO'");
        }
      
//////////////////////Auth code control ////////////////////////////      
      if($AUTH_CODE_CNTL==1){
            $kriter .= $cdb->field_query($kriter, "CDR_ARCHIEVE.AUTH_ID"          ,"<>",       "' '");      
   ...
   $write_to_csv = "";
    ...
    if($SUMM == "trunk" && $TRUNK!=""){
         $sql_str = "   SELECT CDR_ARCHIEVE.CDR_ID,CDR_ARCHIEVE.ORIG_DN,
                        DATE_FORMAT(MY_DATE,\"%d.%m.%Y\") AS MY_DATE, DATE_FORMAT(TIME_STAMP,\"%H:%i:%s\") AS MY_TIME, 
                        CDR_ARCHIEVE.DURATION, CDR_ARCHIEVE.LocationTypeid, EXTENTIONS.DESCRIPTION, 
                        CDR_ARCHIEVE.LocalCode, CDR_ARCHIEVE.CountryCode, CDR_ARCHIEVE.PURE_NUMBER, CDR_ARCHIEVE.Locationid,
                        (CDR_ARCHIEVE.PRICE*$prc_fct) AS PRICE,
                        CDR_ARCHIEVE.TER_TRUNK_MEMBER AS TRUNK, CDR_ARCHIEVE.TER_TRUNK_MEMBER,
                        CDR_ARCHIEVE.AUTH_ID, TIME_STAMP
                   ";
                   
              $sql_str1 = " FROM CDR_ARCHIEVE
                        LEFT JOIN EXTENTIONS ON CDR_ARCHIEVE.ORIG_DN = EXTENTIONS.EXT_NO AND CDR_ARCHIEVE.SITE_ID = EXTENTIONS.SITE_ID
                        LEFT JOIN PHONES ON (CDR_ARCHIEVE.CountryCode = PHONES.COUNTRY_CODE AND 
                                            TRIM(CDR_ARCHIEVE.LocalCode) = TRIM(PHONES.CITY_CODE) AND 
                                            CDR_ARCHIEVE.PURE_NUMBER = PHONES.PHONE_NUMBER AND 
                                            CDR_ARCHIEVE.SITE_ID = PHONES.SITE_ID)
                      LEFT JOIN CONTACTS ON PHONES.CONTACT_ID = CONTACTS.CONTACT_ID ".$provider_join;
         }else{
         $sql_str  = "SELECT CDR_ARCHIEVE.CDR_ID,CDR_ARCHIEVE.ORIG_DN,DATE_FORMAT(CDR_ARCHIEVE.MY_DATE,\"%d/%m/%Y\") AS MY_DATE,
                            DATE_FORMAT(TIME_STAMP,\"%H:%i:%s\") AS MY_TIME,CDR_ARCHIEVE.LocationTypeid,
                            CDR_ARCHIEVE.DURATION, EXTENTIONS.DEPT_ID,CDR_ARCHIEVE.Locationid,
                            CDR_ARCHIEVE.CountryCode, CDR_ARCHIEVE.LocalCode, CDR_ARCHIEVE.PURE_NUMBER, 
                            CDR_ARCHIEVE.AUTH_ID, EXTENTIONS.DESCRIPTION, (CDR_ARCHIEVE.PRICE*$prc_fct) AS PRICE,
                            PHONES.CONTACT_ID, PHONES.DESCRIPTION AS PHONE_DESC, CONTACTS.NAME, CONTACTS.SURNAME, TIME_STAMP
              ";
                        $sql_str1 = " FROM CDR_ARCHIEVE
                            LEFT JOIN EXTENTIONS ON CDR_ARCHIEVE.ORIG_DN = EXTENTIONS.EXT_NO AND CDR_ARCHIEVE.SITE_ID = EXTENTIONS.SITE_ID
                            $fih_join_prm JOIN PHONES ON
                                (CDR_ARCHIEVE.CountryCode = PHONES.COUNTRY_CODE AND 
                                CDR_ARCHIEVE.LocalCode = PHONES.CITY_CODE AND
                                CDR_ARCHIEVE.PURE_NUMBER = PHONES.PHONE_NUMBER AND
                                CDR_ARCHIEVE.SITE_ID = PHONES.SITE_ID)
                            LEFT JOIN CONTACTS ON PHONES.CONTACT_ID = CONTACTS.CONTACT_ID ".$provider_join;
         }
         if ($kriter != ""){
               ...
               if($SUMM == "trunk" && $TRUNK!=""){
                    $sql_str2 .= " AND CDR_ARCHIEVE.TER_TRUNK_MEMBER='".$TRUNK."'";
               }
        }else{
                            ...
                            <tr  <?if ($orig<>'Yes') echo "style=\"display:none;\""?>>
                                ...
                                <?if(is_array($ORIG_ARRAY)){ 
                                    for($i=0;$i<count($ORIG_ARRAY);$i++){
                                        ...
                                        <?echo $ORIG_ARRAY[$i];?> - <? echo get_ext_name($ORIG_ARRAY[$i]).";";}
                                    }
                                   }
                                ?></td>
                            </tr>
...
-->
    <tr>
        <td>
            ...
            ?>
                <tr>
                    <td class="rep_table_header" width="30%" ALIGN="center"><?echo $AUTH_CODE_CNTL==1?"Auth Code":"Dahili"?><a style="cursor:hand;" onclick="javascript:submit_form('Dahili');"><img src="http://<?=$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']?><?=IMAGE_ROOT?><?=($order=="Dahili")?$sort_gif:"sort.gif"?>" align="absmiddle" Title="Dahiliye Göre Sırala"></a></td>
      ...
      ///////////////////////////////////////////////////////////////77
         ...
         $sql_str  = " SELECT CDR_ARCHIEVE.ORIG_DN,EXTENTIONS.DESCRIPTION,
                     DATE_FORMAT(CDR_ARCHIEVE.MY_DATE,\"%d/%m/%Y\") AS MY_DATE,
                     DATE_FORMAT(TIME_STAMP,\"%H:%i:%s\") AS MY_TIME,
                     SEC_TO_TIME(DURATION),
                     CDR_ARCHIEVE.CountryCode, CDR_ARCHIEVE.LocalCode, CDR_ARCHIEVE.PURE_NUMBER,
                     TLocation.LocationName,
                     CONTACTS.NAME, CONTACTS.SURNAME,
                     (CDR_ARCHIEVE.PRICE*1.0) AS PRICE
              ";
        $sql_str1 .=" LEFT JOIN TLocation ON TLocation.Locationid = CDR_ARCHIEVE.Locationid ";
       ...
       rewind($fp);
        ...
        if (!fwrite($fp, "Dahili;Dahili Adı; Tarih; Saat; Süre; Ülke; Şehir; Numara;;;;Ücret \n")) {
                    ...
                    if(is_array($row)) {
                        ...
                        if ($row["CountryCode"] == $local_country_code){
                            ...
                            if ($row["LocalCode"] == $LOC_CODE || $row["LocalCode"]==""){//Şehiriçi Kabul edilen kod.
                              $my_place = "Şehiriçi";
                              $TEL_NUMBER = $LOC_CODE ." ".$row["PURE_NUMBER"];
                            }else{
                              $TEL_NUMBER = $row["LocalCode"]." ".$row["PURE_NUMBER"];
                            }
                        }else{
                            $my_place = $arr_location[$row["Locationid"]];
