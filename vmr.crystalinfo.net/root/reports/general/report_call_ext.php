...
$cdr_table="CDR_MAIN_DATA";
    ...
    $kriter = $cdb->field_query($kriter,   "$cdr_table.SITE_ID"     ,  "=",  "$SITE_ID"); //Bu mutlaka olmalı.İlgili siteyi belirliyor.
    ...
    $kriter .= $cdb->field_query($kriter,   "$cdr_table.ORIG_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
    $kriter .= $cdb->field_query($kriter,   "$cdr_table.ACCESS_CODE"     ,  "=",  "$ACCESS_CODE"); 
...
*/
	...
	$usr_crtd = str_replace("CDR_MAIN_DATA", "$cdr_table", "$usr_crt");
      ...
      <? 
        ...
        $my_pr=0;
         ...
         if(is_array($datas)){    
            ...
            echo  " <tr><td>&nbsp;<b>".$my_dept_name."</b></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";
            unset($dept_totals);
            $i = 0;
            $j++;
            ...
            foreach($value as $keynew=>$valuenew){
				 ...
				 echo  " <td>".$my_dept_name."</td>";
                   $total = 0;
             $csv_data[$j][0] =  $k_x." - ".get_ext_name2($keynew, $SITE_ID);
                   for($k=0;$k<=4;$k++){
                       echo " <td>".write_me($value[$keynew][$k],$calc_type)."</td>";
            ...
            $j++;
            echo  " <tr><td>&nbsp;<b>Alt Toplamlar</b></td>";
			echo  " <td><b>".$my_dept_name."</b></td>";
               for($k=0;$k<=5;$k++){
                   echo " <td><b>".write_me($dept_totals[$k],$calc_type)."</b></td>";
               }
            echo "</tr>";      
      }
          }  
