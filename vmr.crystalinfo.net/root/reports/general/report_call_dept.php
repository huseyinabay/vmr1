...
//   $report_type="GENEL RAPOR";
     if ($act == "src") {
       if ($type=='dept' || $tip=='dept'){ //tip site'dan gelen değişken. Oradaki type ile karıştığı için konuldu.
         $myfld= "PRICE";
         $calc_type =1;   
    ...
    $header="Çağrıların Departmanların Toplam Tutara Göre Dağılımı";
...
<?if($CSV_EXPORT==2){?>
                ...
                <td class="rep_table_header" width="12%">Sehir Ici</td>
                <td class="rep_table_header" width="14%">Sehirler Arasi</td>
                <td class="rep_table_header" width="12%">GSM</td>
                <td class="rep_table_header" width="12%">Uluslar Arasi</td>
                <td class="rep_table_header" width="10%">Diger</td>
...
<?}else{?>
        ...
        <td class="rep_table_header" width="12%">Sehir Ici<a style="cursor:hand;" onclick="javascript:submit_form('0');"><img src="<?=IMAGE_ROOT?><?=($order=="0")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
        <td class="rep_table_header" width="14%">Sehirler Arası<a style="cursor:hand;" onclick="javascript:submit_form('1');"><img src="<?=IMAGE_ROOT?><?=($order=="1")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
        ...
        <td class="rep_table_header" width="12%">Uluslar Arasi<a style="cursor:hand;" onclick="javascript:submit_form('3');"><img src="<?=IMAGE_ROOT?><?=($order=="3")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
        <td class="rep_table_header" width="10%">Diger<a style="cursor:hand;" onclick="javascript:submit_form('4');"><img src="<?=IMAGE_ROOT?><?=($order=="4")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
...
<?}?>
      ...
      <? 
        ...
        $my_pr=0; 
                ...
                $csv_data[0][1] = "Sehir Ici";
                $csv_data[0][2] = "Sehirler Arasi" ;
                $csv_data[0][3] = "GSM";
                $csv_data[0][4] = "Uluslar Arasi";
                $csv_data[0][5] = "Diger";
                $csv_data[0][6] = "Toplam";
         ...
         if(is_array($datas)){   
            foreach($datas as $key=>$value){
                   ...
                   if($key=="0") $key_x = "TANIMSIZ Departman";
                   if($key=="0") $key_y = -2;
...
if($CSV_EXPORT==2){
   $fd = fopen($DOCUMENT_ROOT."/temp/dept_disps.xls", "w");
   fwrite($fd,ob_get_contents());
 }else{
   $fd = fopen($DOCUMENT_ROOT."/temp/dept_disps.html", "w");
   fwrite($fd,ob_get_contents());
 }
  
 ob_end_flush();
 
 csv_out($csv_data, $DOCUMENT_ROOT."/temp/dept_disps.csv"); 

 
 if($CSV_EXPORT==1){?>
 <iframe SRC="/csv_download.php?filename=dept_disps.csv" WIDTH=0 HEIGHT=0 ></iframe>
 <a HREF="/temp/dept_disps.csv">CSV Download</a>
<?}else if($CSV_EXPORT==2){?>
 <iframe SRC="/csv_download.php?filename=dept_disps.xls" WIDTH=0 HEIGHT=0 ></iframe>
  <a HREF="/temp/dept_disps.xls" style="font-size:20px";>Excel XLS Download</a>
 <?}?>
