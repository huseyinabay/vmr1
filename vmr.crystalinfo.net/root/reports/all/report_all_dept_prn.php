    ...
    <tr>
        <td>
            ...
            ?>
                <tr>
<?if($CSV_EXPORT == 2){?>
                    <td class="rep_table_header" width="10%" ALIGN="center">Departman</td>
...
<?}else{?>
                    <td class="rep_table_header" width="10%" ALIGN="center">Departman<a style="cursor:hand;" onclick="javascript:submit_form('dept');"><img src="http://<?=$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']?><?=IMAGE_ROOT?><?=($order=="dept")?$sort_gif:"sort.gif"?>" align="absmiddle" Title="Departmana Göre Sırala"></a></td>
<?  if($AUTH_CODE_CNTL==1){?>
...
<?}?>
            ...
            if (mysqli_num_rows($result)>0)
                ...
                switch ($order){
                    ...
                    case 'ucret':
                        ...
                        break;
                    default:
                        $order1='DEPT_ID, ORIG_DN, MY_DATE';
                }
                ...
                while($row_array[$cnt] = mysqli_fetch_array($result)){$cnt++;}
                    ...
                    if(is_array($row)) {
                        ...
                        $dept_arr[$row["DEPT_ID"]][0] = $dept_name;
                        $dept_arr[$row["DEPT_ID"]][1] = $dept_arr[$row["DEPT_ID"]][1]+$row["PRICE"];
                        $dept_arr[$row["DEPT_ID"]][2] = $dept_arr[$row["DEPT_ID"]][2]+1;
                        $csv_data[0] = $dept_name;
                        if($AUTH_CODE_CNTL==1){
                          $csv_data[1] = substr($arr_auth_code[$row["AUTH_ID"]],0,18)."(".$row["ORIG_DN"].")";
                          $ext_arr[$row["AUTH_ID"]][0] = substr($arr_auth_code[$row["AUTH_ID"]],0,18)."(".$row["ORIG_DN"].")";
                          $ext_arr[$row["AUTH_ID"]][1] = $ext_arr[$row["AUTH_ID"]][1]+$row["PRICE"];
                          $ext_arr[$row["AUTH_ID"]][2] = $ext_arr[$row["AUTH_ID"]][2]+1;
                        }else{
                          $csv_data[1] = $row["ORIG_DN"]." - ".substr($row["DESCRIPTION"],0,18);
                          $ext_arr[$row["ORIG_DN"]][0] = substr($row["DESCRIPTION"],0,18);
                          $ext_arr[$row["ORIG_DN"]][1] = $ext_arr[$row["ORIG_DN"]][1]+$row["PRICE"];
                          $ext_arr[$row["ORIG_DN"]][2] = $ext_arr[$row["ORIG_DN"]][2]+1;
                        }
                        $csv_data[2] = $row["MY_DATE"];
                        $csv_data[3] = $row["MY_TIME"] ;
                        $csv_data[4] = calculate_all_time($row["DURATION"]);
                        ...
                        $csv_data[5] = $TEL_NUMBER;
                        $csv_data[6] = str_replace("</b>","",str_replace("<b>","",$called));
                        $csv_data[7] = write_price($row["PRICE"]);
                        ...
                        if($CSV_EXPORT<>1){
                            ...
                            echo " <td class=\"rep_td\">".$dept_name."</td>";
    ...
    <tr>
      <td class="rep_table_header" colspan="6">KULLANICI (TEL) TOPLAM</td>
      <td class="rep_table_header" colspan="1">Görüşme Adedi</td>
      <td class="rep_table_header" colspan="1">&nbsp;</td>
    </tr>
<?
        $csv_data[0] = "KULLANICI (TEL) TOPLAM";
        $csv_data[1] = "";
        $csv_data[2] = "";
        $csv_data[3] = "";
        $csv_data[4] = "";
        $csv_data[5] = "";
        $csv_data[6] = "GÖRÜŞME ADEDİ";
        $csv_data[7] = "";
        if($CSV_EXPORT==1){
          csv_write_line($csv_data, dirname($DOCUMENT_ROOT)."/root/temp/$file_name.csv", $fp);
        }
        if(is_array($ext_arr)){    
            ...
            foreach($ext_arr as $key=>$value){
              ...
              if($CSV_EXPORT<>1){
                echo "<tr bgcolor=\"$bg_color\">"
            ?>
                <td class="rep_td" colspan="6"><?=$key?> - <?=$value[0]?></td>
                <td class="rep_td" colspan="1"><?=$value[2]?></td>
                <td class="rep_td" colspan="1" align="right"><?=write_price($value[1])?></td>
                </tr>
<?           } 
                $csv_data[0] = $key." - ".$value[0];
                $csv_data[1] = "";
                $csv_data[2] = "" ;
                $csv_data[3] = "";
                $csv_data[4] = "";
                $csv_data[5] = "";
                $csv_data[6] = $value[2];
                $csv_data[7] = write_price($value[1]);
                if($CSV_EXPORT==1){
                    csv_write_line($csv_data, dirname($DOCUMENT_ROOT)."/root/temp/$file_name.csv", $fp);
                }
             }
...
?>
    ...
    <tr><td height="22" bgcolor="FFFFFF" colspan="8">&nbsp;</td></tr>        
    <tr>
      <td class="rep_table_header" colspan="8">DEPARTMAN TOPLAM</td>
    </tr>
<?
        $csv_data[0] = "DEPARTMAN TOPLAM";
        $csv_data[1] = "";
        $csv_data[2] = "";
        $csv_data[3] = "";
        $csv_data[4] = "";
...
<?
                ...
                $csv_data[3] = "";
                $csv_data[4] = "";
                $csv_data[5] = "Toplam Görüşme Adedi";
                $csv_data[6] = "Toplam Süre ";
                $csv_data[7] = "Toplam Ücret";
                $m++;
