<?
   ...
   $conn = $cdb->getConnection();
    ...
    exit;
     ...
     if ($act=="upd" && $id!=="" && is_numeric($id)){
       ...
       $sql_str = "SELECT ALERT_DEF_NAME FROM ALERT_DEFS WHERE ALERT_DEFS.ALERT_ID = 4
  ...
  cc_page_meta();
     ...
     echo "<center>";
     echo "<br>";
     table_header("Ücret Uyarıları","80%");
?>
<script>function submit_form() {
   if(check_form(document.price_alert)){
        document.all('SITE_ID').disabled=false;   
        document.price_alert.submit();
   }
}
</script>
  <center>
  <table width="65%" cellpadding="0" cellspacing="0" align="center">
      <form name="price_alert" method="post" onsubmit="return check_form(this);" action="price_alert_db.php?act=<? echo  $_GET['act'] ?>">
      ...
      <tr class="form">
          <td width="30%" class="td1_koyu">Uyarı Adı</td>
           <td width="70%"><input size="40" class="input1" maxlenght="250" name="ALERT_DEF_NAME" VALUE="<?=$ALERT_DEF_NAME;?>">  </td> 
       </tr>
     <tr class="form">
          <td width="30%" class="td1_koyu">Konuşma Ücreti</td>
          <td width="70%"><input size="15" class="input1" maxlenght="20" name="PRICE" VALUE="<?=$PRICE;?>">  </td> 
         </tr>
    <tr height="40">
      <td></td>
  ...
  <table width="65%" cellpadding="0" cellspacing="0" border="0" align="center">
        ...
        <table>      
          ...
          if ($act=='upd'){
            ...
            echo " <tr class=\"".bgc2($i)."\">";
                    ...
                    echo "<td width=\"10%\"><a href=\"javascript:popup('add_email.php?act=upd&id=".$row1->AEM_ID."&ALERT_DEF_ID=$id','price_alert',500,200)\"><img border=\"0\" src=\"".IMAGE_ROOT."guncelle1.gif\"></a></td>";
                    echo "<td width=\"5%\"><a href=\"javascript:if(confirm('Dikkat  E-mail silinecek'))popup('add_email.php?act=del&id=".$row1->AEM_ID."','price_alert',500,200)\"><img border=\"0\" src=\"".IMAGE_ROOT."email_sil.gif\"></a></td>";
            echo "</tr>";
            $i++;
            }
          }?>
    ...
    <tr>
      ...
      <tr class="form">
          <td colspan="2"class="td1"><br>Yukarıda girilmiş olan ücretin üzerinde konuşma yapıldığında belirtilen kişilere
  ...
  <table align="right" width="100%" border="0">
    <tr>
      ...
      <td align="right"><a href="price_alert_src.php"><img border="0" src="<?=IMAGE_ROOT?>arama_yap.gif" style="cursor:hand;"></a></td>
      <td align="right"><a href="price_alert.php?act=new"><img border="0" src="<?=IMAGE_ROOT?>yeni_kayit.gif" style="cursor:hand;"></a></td>
       <?if ($id){?>
      <td align="right"><a href="javascript:if(confirm(' Dikkat \n Kayıt Silinecek')) this.location.href='price_alert_db.php?act=del&id=<?=$id?>'"><img border="0" src="<?=IMAGE_ROOT?>kayit_sil.gif" style="cursor:hand;"></a></td>
         <?}?>
    </tr>
  </table>
      ...
      <script language="javascript">
            form_fields[0] = Array ("SITE_ID", "Site adını seçmeniz gerekli.", TYP_DROPDOWN);
            form_fields[1] = Array ("ALERT_DEF_NAME", "Uyarı adını girmeniz gerekli.", TYP_NOT_NULL);
            form_fields[2] = Array ("PRICE", "Konuşma ücretini girmeniz gerekli.", TYP_NOT_NULL);
