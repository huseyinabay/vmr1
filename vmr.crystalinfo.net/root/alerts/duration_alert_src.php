<?
  ...
  if ($SITE_ID<>'-1' && $SITE_ID<>""){
    ...
    $sql_str = "SELECT ALERT_DEFS.ALERT_DEF_NAME,ALERT_DEFS.ALERT_DEF_ID FROM ALERT_DEFS 
        WHERE ALERT_DEFS.ALERT_ID = 2 ".$site_crt;
...
<script langoage="javascript">
  function submit_me(){
    document.site_src.action = 'duration_alert_src.php?act=src&SITE_ID=' + document.all('SITE_ID').value;
    document.site_src.submit();
  }
</script>
<center>
<a href="duration_alert.php?act=new"><img border="0" src="<?=IMAGE_ROOT?>yeni_kayit.gif" style="cursor:hand;"></a>
<form name="site_src" method="post" action="duration_alert_src.php">
   ...
   while($row = mysqli_fetch_object($result)){
     ...
     while ($row1 = mysqli_fetch_object($result1)){
      ...
      echo " <td height=\"20\">$row->ALERT_DEF_ID</td>".CR;
       ...
       echo " <td>".substr($row->ALERT_DEF_NAME,0,25)."</td>".CR;
        echo " <td>$dur</td>".CR;
       echo " <td><a HREF=\"duration_alert.php?act=upd&id=$row->ALERT_DEF_ID\">Güncelle</td>".CR;
         echo "</tr>".CR;
       list_line(18);
   }
?>
