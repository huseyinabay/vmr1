<?
   ...
   $conn = $cdb->getConnection();
    ...
    if (right_get("ADMIN") && !right_get("SITE_ADMIN")){
      $site_crt = " AND ALERT_DEFS.SITE_ID = ".$_SESSION['site_id'];
       ...
       $sql_str = "SELECT * FROM ALERT_DEFS WHERE ALERT_DEFS.ALERT_ID = 8
    ...
    $ALERT_DEF_NAME = 'Ayın Konuşma Özeti';
  }
  ...
  cc_page_meta();
     ...
     echo "<center>";
     echo "<br>";
     table_header("Ay Özeti","80%");
?>
<script>
    function submit_form() {
        if(check_form(document.month_summary_alert)){
          document.all('SITE_ID').disabled=false;      
            document.month_summary_alert.submit();
        }
    }
</script>
  <center>
  <table width="50%" cellpadding="0" cellspacing="0">
    <form name="month_summary_alert" method="post" onsubmit="return check_form(this);" action="month_summary_alert_db.php?act=<? echo  $_GET['act'] ?>">
  ...
  <table width="50%" cellpadding="0" cellspacing="0" border="0" align="center">
              ...
              if(mysqli_num_rows($result1)>0){
                while ($row1 = mysqli_fetch_object($result1)){
                  echo " <tr class=\"".bgc2($i)."\">";
                          ...
                          echo "<td width=\"10%\"><a href=\"javascript:popup('add_email.php?act=upd&id=".$row1->AEM_ID."&ALERT_DEF_ID=$ALERT_DEF_ID','month_summary_alert',500,200)\"><img border=\"0\" src=\"".IMAGE_ROOT."guncelle1.gif\"></a></td>";
                          echo "<td width=\"5%\"><a href=\"javascript:if(confirm('Dikkat  E-mail silinecek'))popup('add_email.php?act=del&id=".$row1->AEM_ID."','month_summary_alert',500,200)\"><img border=\"0\" src=\"".IMAGE_ROOT."email_sil.gif\"></a></td>";
                  echo "</tr>";
                  $i++;
                }
              }
    ...
    <tr>
      ...
      <tr class="form">
          <td colspan="2"class="td1"><br>Ayın Konuşma Özeti belirtilen e-posta adresine
  ...
  <table align="right" width="100%" border="0">
    <tr>
      ...
      <td align="right"><a href="month_summary_alert_src.php"><img border="0" src="<?=IMAGE_ROOT?>arama_yap.gif" style="cursor:hand;"></a></td>
      <td align="right"><a href="month_summary_alert.php?act=new"><img border="0" src="<?=IMAGE_ROOT?>yeni_kayit.gif" style="cursor:hand;"></a></td>
         <?if ($id){?>
      <td align="right"><a href="javascript:if(confirm(' Dikkat \n Kayıt Silinecek')) this.location.href='month_summary_alert_db.php?act=del&id=<?=$id?>'"><img border="0" src="<?=IMAGE_ROOT?>kayit_sil.gif" style="cursor:hand;"></a></td>
         <?}?>
