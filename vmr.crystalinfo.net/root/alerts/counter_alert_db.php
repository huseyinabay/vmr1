<?
  ...
  //Site Admin veya Admin HakkÄ± yokda bu tanÄ±mÄ± yapamamalÄ±
  if (!right_get("SITE_ADMIN") && !right_get("ADMIN")){
    print_error("Bu sayfaya eriÅŸim hakkÄ±nÄ±z yok!");
    exit;
  }

  //Bir uyarÄ± silinecekse onun kriterleri ve gÃ¶nderilecek E-mail adresleri de silinmeli.
  if($act == "del" && $id !="" && is_numeric($id)){
     ...
     $sql_str = "DELETE FROM ALERT_TO_EMAIL WHERE ALERT_DEF_ID = $id"; 
      if (!($cDB->execute_sql($sql_str,$result,$error_msg))){
         ...
         header("Location:counter_alert_src.php");
     exit;
  }else{
    //UyarÄ± eklendiÄŸinde veya deðiþtirildiðinde mutlaka yapýldýðý andan sonraki
    //kayýtlar için iþlemeli. Geçmiþ kayýtlarla ilgilenmemeli.
    ...
    //act=upd ise kayýtlarýn bulunmasýnda sorun olabileceði için mevcut kayýtlar siliniyor
    ...
    //Yeni kayýtsa o kayýt ile ilgili bilgiler girilmeli.
     if ($act=='new' || $act==''){ 
         $args = Array();
      $args[] = array("ALERT_ID",           '3',           cFldWoQuote);
      $args[] = array("ALERT_DEF_NAME",        $ALERT_DEF_NAME,   cFldWQuote);
          $args[] = array("LAST_PROC_ID",        $LAST_PROC_ID,     cFldWQuote);
          $args[] = array("SITE_ID",          $SITE_ID,       cFldWQuote);
    ...
    $args2[] = array("ALERT_DEF_ID",   $id,      cFldWoQuote);
     $args2[] = array("FIELD_NAME",       'SITE_ID',       cFldWQuote);
      ...
      $args2[] = array("VALUE",          $SITE_ID,       cFldWQuote);
    ...
    $args2[] = array("ALERT_DEF_ID",   $id,      cFldWoQuote);
         $args2[] = array("FIELD_NAME",       'COUNTER',       cFldWQuote);
         $args2[] = array("OPERATOR",        '>',       cFldWQuote);
         $args2[] = array("VALUE",          $COUNTER,       cFldWQuote);
    ...
    header("Location:counter_alert.php?act=upd&saved=1&id=".$id);
?>
