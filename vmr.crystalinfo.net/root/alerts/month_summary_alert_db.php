<?
  ...
  //Site Admin veya Admin Hakkı yokda bu tanımı yapamamalı
   if (!right_get("SITE_ADMIN") && !right_get("ADMIN")){
    ...
    exit;
      ...
      $sql_str = "SELECT * FROM ALERT_DEFS WHERE ALERT_ID = 8 AND SITE_ID = '$SITE_ID'" ; 
  ...
  if($act == "del" && $id !="" && is_numeric($id)){
     ...
     $sql_str = "DELETE FROM ALERT_TO_EMAIL WHERE ALERT_DEF_ID = $id"; 
      ...
      header("Location:month_summary_alert_src.php");
    exit;
  }else{
    ...
    //Yeni kayıtsa o kayıt ile ilgili bilgiler girilmeli.
     if ($act=='new' || $act==''){ 
         $args = Array();
      $args[] = array("ALERT_ID",           '8',             cFldWoQuote);
      $args[] = array("ALERT_DEF_NAME",        'Ayın Konuşma Özeti',   cFldWQuote);
          $args[] = array("SITE_ID",          $SITE_ID,         cFldWQuote);
      $args[] = array("FREQUENCY",        '3',             cFldWoQuote);
    ...
    header("Location:month_summary_alert.php?act=upd&saved=1&id=".$id);
?>
