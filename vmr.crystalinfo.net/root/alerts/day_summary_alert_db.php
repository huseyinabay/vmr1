<?
  ...
  //Site Admin veya Admin Hakkı yokda bu tanımı yapamamalı
   if (!right_get("SITE_ADMIN") && !right_get("ADMIN")){
    ...
    exit;
      ...
      $sql_str = "SELECT * FROM ALERT_DEFS WHERE ALERT_ID = 7 AND SITE_ID = '$SITE_ID'" ; 
      $my_state = exist_status($sql_str,$act,'ALERT_DEF_ID',$id);
      if ($my_state==1 || $my_state==2){//1 yeni kayıt ve mevcut 2 update ve mevcut
      print_error("Bu site için Günlük Rapor kaydı sistemde mevcuttur.");
            exit;
      }      
  ...
  if($act == "del" && $id !="" && is_numeric($id)){
     ...
     $sql_str = "DELETE FROM ALERT_TO_EMAIL WHERE ALERT_DEF_ID = $id"; 
      ...
      header("Location:day_summary_alert_src.php");
    exit;
  }else{
    ...
    //Yeni kayıtsa o kayıt ile ilgili bilgiler girilmeli.
     if ($act=='new' || $act==''){ 
         $args = Array();
      $args[] = array("ALERT_ID",           '7',             cFldWoQuote);
      $args[] = array("ALERT_DEF_NAME",        'Günün Konuşma Özeti',   cFldWQuote);
          $args[] = array("SITE_ID",          $SITE_ID,         cFldWQuote);
      $args[] = array("FREQUENCY",        '1',             cFldWoQuote);
    ...
    header("Location:day_summary_alert.php?act=upd&saved=1&id=".$id);
?>
