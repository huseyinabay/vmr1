<?
   ...
   $act = "src";
   if ($act == "src") {
     $sql_str  = "SELECT * FROM TProviderTariff WHERE ProviderTariffid<7";
...
?>
<form name="sysprm_arama" method="post" action="tariff_ucret_src.php?act=src">
...
<?  
   ...
   while($row = mysqli_fetch_object($rs)){
      $i++;
      echo "<tr class=\"".bgc($i)."\" title=\"$row->TariffName : $row->TariffName\">".CR;
      echo " <td height=\"20\">$row->ProviderTariffid</td>".CR;
      echo " <td>$row->TariffName</td>".CR;
      echo " <td>$row->CounterPrc</td>".CR;
      echo " <td><a HREF=\"#\"  onclick=\"javascript:popup('tariff_ucret.php?act=upd&id=$row->ProviderTariffid','system',450,250)\">Güncelle</td>".CR;
      echo "</tr>";
          list_line(18);
   }
?>
