<?php
     ...
     ///İkinci fetch. Sonraki ay alınıyor
     $row = mysqli_fetch_array($result);
     ...
     $labels[] = turkish2utf("Diğer");
     $labels[] = turkish2utf("U.Arası");
     $labels[] = "GSM";
     $labels[] = turkish2utf("Ş.Arası");
     $labels[] = turkish2utf("Ş.İçi");

	 $title = turkish2utf($onceki."-".$gecen." Tutar Dağılımı");
     $titleObj = $c->addTitle($title, "verdana.ttf", 14);
     $titleObj->setBackground(0x9999ff);
     $c->setPlotArea(80, 35, 370, 200, 0xffffff);
	 $c->swapXY();
     $c->yAxis->setTitle("Tutar (Bin)", "verdana.ttf", 11);
     $c->xAxis->setTickLength(7);
	 $c->yAxis->setTopMargin(20);
     $myxAxis = $c->xAxis->setLabels($labels);
     $c->xAxis->setTitle(turkish2utf(" Çağrı Türü "),"verdana.ttf", 14);
