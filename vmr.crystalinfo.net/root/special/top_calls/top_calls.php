...
//departmanın dahili numaralarını alarak bir kriter oluşturmak için kullanılır.
    ...
    $SQL_SITE = "SELECT SITE_ID, SITE_NAME, MAX_ACCE_DURATION FROM SITES WHERE SITE_ID=".$SITE_ID;
...
/*          GELEN ÇAĞRILAR         */            
            ...
            echo $DATA1; //die;
             if($CSV_EXPORT==1){
               $fd = fopen($DOCUMENT_ROOT."/temp/top_calls.xls", w);
             }else{
               $fd = fopen($DOCUMENT_ROOT."/temp/top_calls.html", w);
             }  
...
?>
<?if($CSV_EXPORT==1){?>
 <iframe SRC="/csv_download.php?filename=top_calls.xls" WIDTH=0 HEIGHT=0 ></iframe>
              <br><br><br>
              <p align="center"> <a HREF="/temp/top_calls.xls">Excel Download</a></p>
 <?}else{?>
<center><input type="button" value="E-mail Gönder" onclick="javascript:mailPage('top_calls.html')"></center>
 <script language="JavaScript">
