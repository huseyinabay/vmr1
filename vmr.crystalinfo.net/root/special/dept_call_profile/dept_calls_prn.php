...
<?}?>
          ...
          <TD width="50%" align=center CLASS="header"><?echo $company;?><BR>AYLIK DEPARTMAN ÇAĞRI PROFİLİ<br><BR><?=get_site_name($SITE_ID)?></TD>
...
<?
      ...
      while($row = mysqli_fetch_object($result)){
         ...
         if (!($cdb->execute_sql($sql_inb, $rsltinb, $error_msg))){
          ...
          $row_inb = mysqli_fetch_object($rsltinb);
...
<?
    ...
    $row_total = mysqli_fetch_object($rslttotal);
    $SITE_TOTAL = $row_total->LOC_PRICE+$row_total->NAT_PRICE+$row_total->GSM_PRICE+$row_total->INT_PRICE+$row_total->OTH_PRICE;
?>  
  <tr height="20">
    <td>
    <b>Departmanın Site içerisindeki maliyet oranı %: <?=round(($tEXT_TOTAL_PRICE/$SITE_TOTAL)*100,2)?></b> 
    </td>
  </tr>
  <tr height="20">
    <td><BR><BR>
    <b>Not : <?=strftime("%B", mktime(0, 0, 0, $mymonth-1 ,32, $myyear))?> ayı iş günü sayısı <?=$job_days?> gün olarak hesaplanmıştır.</b> 
    <br><br><br>
    </td>
  </tr>
</table>        
