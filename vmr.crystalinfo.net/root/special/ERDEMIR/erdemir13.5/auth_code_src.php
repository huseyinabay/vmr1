<?  require_once $_SERVER['DOCUMENT_ROOT']."/cgi-bin/functions.php";
  ...
  //Site Admin veya Admin Hakki yokda bu tanimi yapamamali
     if (!right_get("SITE_ADMIN") && !right_get("ADMIN")){
        print_error("Bu sayfaya erisim hakkiniz yok!");
    exit;
     }
  //Site Admin Hakki Yoksa sadece kendisine bagli kayitlari görsün   
...
<center>
       <form name="auth_code_src" method="post" action="auth_code_src.php?act=src">
         ...
         <table cellpadding="0" cellspacing="0" align="center" width="75%">
            <tr class="form">
                <td class="font_beyaz">Site Adi</td>
                <td>
...
<?
   ...
   if ($act == "src") {
         ...
         $kriter .= $cdb->field_query($kriter, "AUTH_CODE",     "LIKE", "'$AUTH_CODE%'");
         $kriter .= $cdb->field_query($kriter, "ACCOUNT_NO",     "LIKE", "'%$ACCOUNT_NO%'");
         $kriter .= $cdb->field_query($kriter, "AUTH_CODE_DESC", "LIKE", "'%$AUTH_CODE_DESC%'");
      
         $sql_str  = "SELECT AUTH_CODES.AUTH_CODE_ID,AUTH_CODES.AUTH_CODE,AUTH_CODES.AUTH_CODE_DESC, AUTH_CODES.ACCOUNT_NO, 
                        SITES.SITE_NAME
                      FROM AUTH_CODES
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr class="header_beyaz">
        ...
        <td><a href="javascript:submit_form('auth_code_src',1, 'SITES.SITE_NAME')">Site Adi</a></td>        
        <td><a href="javascript:submit_form('auth_code_src',1, 'AUTH_CODES.AUTH_CODE')">Auth.Kodu</a></td>
        <td><a href="javascript:submit_form('auth_code_src',1, 'AUTH_CODES.ACCOUNT_NO')">Hesap No</a></td>
        <td><a href="javascript:submit_form('auth_code_src',1, 'AUTH_CODES.AUTH_CODE_DESC')">Açiklama</a></td>
        <td>Güncelle</td>
    </tr>
<? 
   $i;
