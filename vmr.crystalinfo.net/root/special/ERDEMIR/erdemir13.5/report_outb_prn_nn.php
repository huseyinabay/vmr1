  ...
  $sql_str="SELECT Locationid,LocationName FROM TLocation"; 
   ...
   if ($act == "src") {
    ...
    if($in_str != ""){
		...
		$kriter .= $cdb->field_query($kriter, "CDR_MAIN_DATAACCOUNT_NO"        ,"like",     "'%$ACCOUNT_NO%'");
/*        if($MEMBER_NO != -1){
   ...
   $write_to_csv = "";
    ...
    if($SUMM == "trunk" && $TRUNK!=""){
         $sql_str = "   SELECT CDR_MAIN_DATA.CDR_ID,LTRIM(CDR_MAIN_DATA.ORIG_DN) AS ORIG_DN,
                        ...
                        CDR_MAIN_DATA.AUTH_ID, EXTENTIONS.ACCOUNT_NO, TIME_STAMP, DEPTS.DEPT_NAME
                   ";
         ...
         $sql_str  = "SELECT CDR_MAIN_DATA.CDR_ID,LTRIM(CDR_MAIN_DATA.ORIG_DN) AS ORIG_DN,DATE_FORMAT(CDR_MAIN_DATA.MY_DATE,\"%d/%m/%Y\") AS MY_DATE,
                            ...
                            CDR_MAIN_DATA.AUTH_ID, EXTENTIONS.ACCOUNT_NO, EXTENTIONS.DESCRIPTION, (CDR_MAIN_DATA.PRICE*$prc_fct) AS PRICE,
