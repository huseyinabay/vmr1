<?
 ...
 //Site Admin veya Admin Hakki yokda bu tanimi yapamamali
   ...
   if ($act=="upd" && $id!="" && is_numeric($id)){
    //Admin Hakki versa ve Site Admin hakki yoksa sadece kendi sitesine ait bilgiyi görebilmeli
    ...
    if (mysqli_numrows($result)>0){
        ...
        print_error("Belirtilen Kayit Bulunamadi");
          exit;
    }
   }
   
//Action upd ise SITE_ID db den gelen degilse Session'dan gelen olmali.
  if($act=='upd')
     $SITE_ID = $row->SITE_ID;
  else
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
           ...
           <tr class="form">
                ...
                <tr class="form">
                  <td class="td1_koyu">E mail</td>
                   <td><input type="text" class="input1" size="30" name="EMAIL" VALUE="<?echo $row->EMAIL?>" Maxlength="75"></td> 
               </tr>
                <tr class="form">
                  <td class="td1_koyu">Hesap No</td>
                   <td><input type="text" class="input1" size="30" name="ACCOUNT_NO" VALUE="<?echo $row->ACCOUNT_NO?>" Maxlength="15"></td> 
               </tr>
               <tr class="form">
                  <td class="td1_koyu">Açiklama</td>
  ...
  <table align="right" width="100%" border="0">
    ...
    <script language="javascript">
            form_fields[0] = Array ("SITE_ID",   "Authorizasyon Kodunun Bagli Oldugu Siteyi Seçiniz.", TYP_DROPDOWN);            
