...
/*       Rapor gönderilecek mail adreslerini çek       */    
     ...
     $sql_mail = "SELECT EMAIL AS TO_EMAIL, AUTH_CODE from AUTH_CODES WHERE EMAIL <>''";
     if (!($cdb->execute_sql($sql_mail, $res_mail, $error_msg))){
       print_error($error_msg);
       exit;
    }

    while($rw_mail = mysqli_fetch_object($res_mail)){
       $file_name = "Telefon_" . md5(uniqid(rand(), true)).".txt";

        getAccountDetail($rw_mail->AUTH_CODE);
        sleep(2);        
        mail_send_erdemir($rw_mail->TO_EMAIL,"*** ERDEMIR OZEL GORUSMELER TELEFON RAPORU  ***", "*** ERDEMIR OZEL GORUSMELER TELEFON RAPORU  ***",0,$file_name);
    ...
    if (right_get("SITE_ADMIN")){
        //Site admin hakki varsa herseyi görebilir. 
        //Site id gelmemisse kisinin bulundugu site raporu alinir.
        if(!$SITE_ID){$SITE_ID = $_SESSION['site_id'];}
    }elseif(right_get("ADMIN") || right_get("ALL_REPORT")){
        // Admin vaye ALL_REPORT hakki varsa kendi sitesindeki herseyi görebilir.
        $SITE_ID = $_SESSION['site_id'];
    }elseif(got_dept_right($_SESSION["user_id"])==1){
        //Bir departmanin raporunu görebiliyorsa kendi sitesindekileri girebilir.
        $SITE_ID = $_SESSION['site_id'];
...
function getAccountDetail($accountNo){
   global $SITE_ID,$cdb,$DOCUMENT_ROOT,$raporlanacak_gun_sayisi, $file_name;
    ...
    //Raporlanmak istenmeyen dahililer aliniyor.
  ...
  cc_page_meta();
    ...
    //Joinden kaçmak için Lokasyon tablosundaki bilgiler aliniyor.
  $sql_str="SELECT Locationid,LocationName FROM TLocation"; 
    ...
    //Joinden kaçmak için Çagri türündeki tablosundaki bilgiler aliniyor.
    ...
    //Joinden kaçmak için Auth_Code tablosundaki bilgiler aliniyor.
    ...
    if (mysqli_num_rows($result1)>0){
        ...
        print_error("Site paramatreleri bulunamadi.");
        exit;
    }
    $local_country_code = get_country_code($SITE_ID);//Lokal ülke kodu.
	
   $report_type="Giden Çagri Raporu";
   ...
   if ($act == "src") {
      ...
      //Temel kriterler. Verinini hizli gelmesi için basa konuldu.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.SITE_ID"     ,  "=",  "$SITE_ID"); //Bu mutlaka olmali.Ýlgili siteyi belirliyor.
      $kriter .= $cdb->field_query($kriter,   "ERR_CODE"     ,  "=",  "0"); //Bu mutlaka olmali.Hatasiz kayit oldugunu gösteriyor.        
      $kriter .= $cdb->field_query($kriter,   "CALL_TYPE"     ,  "=",  "1"); //Bu mutlaka olmali.Dis arama oldugunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "DURATION"     ,  "<",  "$max_acc_dur"); //Bu mutlaka olmali.Dis arama oldugunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ORIG_DN"     ,  " <> ",  " '' "); //Bu mutlaka olmali.Hatasiz kayit oldugunu gösteriyor.

      //**Bunlar birlikte olmali ve bu sirada olmali.
     ...
     //Genel Arama Bilgileri. Bu alanlar baslik basilmasinda kullanilacaktir.
    if ($ORIG_DN <> '') 
        $orig = 'Yes';
    ...
    $provider_join="";//Ne olur ne olmaz bu alan burada bosaltilsin.
    ...
    if(is_array($DEPT_ID)){
        ...
        $kriter .= $cdb->field_query($kriter, "CDR_MAIN_DATA.AUTH_ID"          ,"=",        "'$accountNo'");
...
//////////////////////Auth code control ////////////////////////////      
    ...
    $fih_join_prm = " LEFT ";
    $fih_where_prm = "";
    //Fihrist kayitlai isteniyorsa Contacts ile INNER JOIN olmali
    $fihrist_state = "";
    if($IN_FIHRIST && !$OUT_FIHRIST){
        $fih_join_prm = " INNER ";
        if($CONTACT_TYPE == '0' || $CONTACT_TYPE == '1'){
            $kriter .= $cdb->field_query($kriter, "CONTACTS.IS_GLOBAL"          ,"=",       "'$CONTACT_TYPE'");
            if($CONTACT_TYPE == '1'){$fihrist_state = "Þirket Kontaklari";}
            else{$fihrist_state = "Özel Kontaklar";}
        } else{$fihrist_state = "Tüm Kayitli Olanlar";}  
    }
    if(!$IN_FIHRIST && $OUT_FIHRIST){
      $fih_where_prm = " AND PHONES.COUNTRY_CODE IS NULL";
      $fihrist_state = "Kayitli Olmayanlar";
    }
    //Fihirist ile ilgili birsey seçildiyse kriter gelmeli yoksa gerek yok.
    //Fihrist disi kayitlar ise joinde karsiligi null gelecektir. Contacktlarda 
    //Country code mutlaka dolu oldugundan bu alan bossa fihrist karsiligi yoktur.
   $write_to_csv = "";
    //Trunk raporlari üzerinden gelmekte. Bir trunktan yapilan aramalar.
...
#####################################################
      ...
      ///////////////////////////////////////////////////////////////77
         ...
         //Gelen kayit adedi çok fazla ise bunlar csv ile kaydedilsin.
            $CSV_EXPORT = 1;
            ...
            if (mysqli_num_rows($result)>0)
                ...
                if($CSV_EXPORT==1)
                 ...
                 echo "<table border=0 width=400>\n";
                     ...
                     if($CSV_EXPORT==1){
                        while($rowd = mysqli_fetch_object($result)){                     
                        ...
                        while($rowd = mysqli_fetch_object($result)){
                          ...
                          if($old_account_no<>$rowd->ACCOUNT_NO){ 
                            echo "<tr>";
                            echo "<td colspan=5>*** Erdemir Özel Telefon Faturasi ***   	Hesap Kodu :".$rowd->ACCOUNT_NO."</td></tr><tr>\n";
                          }
                          if($old_ext_no<>$rowd->EXT_NO){ 
...
<?
  ...
  function mail_send_erdemir($mail,$subject,$DATA,$add_img=1, $file_name=""){
	 ...
	 if ($DATA!=""){
        ...
        $Mail->IsSmtp();
        if (!$Mail->Send()){
            echo $Mail->ErrorInfo. "<br>". $mail ." Adresine Atamadim. \n";
        }else{
           echo $mail ." Adresine Atildi. \n<BR>";
        }
