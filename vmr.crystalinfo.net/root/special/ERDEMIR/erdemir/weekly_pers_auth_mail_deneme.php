...
/*       Rapor gönderilecek mail adreslerini çek       */    
    ...
    while($rw_mail = mysql_fetch_object($res_mail)){
       $file_name = "Telefon_" . md5(uniqid(rand(), true)).".txt";
        ...
        sleep(2);        
        mail_send_erdemir($rw_mail->TO_EMAIL,"*** ERDEMIR OZEL TELEFON RAPORU  ***", "*** ERDEMIR OZEL TELEFON RAPORU  ***",0,$file_name);
  ...
  $sql_str="SELECT Locationid,LocationName FROM TLocation"; 
   ...
   if ($act == "src") {
    ...
    if(is_array($DEPT_ID)){
        ...
        $kriter .= $cdb->field_query($kriter, "CDR_MAIN_DATA.DIGITS"      ,"LIKE",     "'%$DIGITS%'");
...
//////////////////////Auth code control ////////////////////////////      
   ...
   $sql_str1 = " FROM $CDR_MAIN_DATA AS CDR_MAIN_DATA
               ...
               CDR_MAIN_DATA.DIGITS = DIGITS AND
...
#AUTH   CODE LARA GÖRE RAPOR ALMAK ÝÇÝN
   ...
   $sql_str_a  = "SELECT CONCAT(CONTACTS.NAME,' ', CONTACTS.SURNAME) AS DESTINATION,  ACCOUNT_NO, AUTH_ID, PRICE, DATE_FORMAT(CDR_MAIN_DATA.MY_DATE,\"%d/%m\") AS TXTMY_DATE,
                            DATE_FORMAT(TIME_STAMP,\"%H:%i\") AS TXTMY_TIME,CDR_MAIN_DATA.CountryCode, CDR_MAIN_DATA.LocalCode, CDR_MAIN_DATA.DIGITS, CDR_MAIN_DATA.COUNTER,2 AS TYPE, CDR_MAIN_DATA.DURATION,AUTH_CODE_DESC AS DESCRIPTION ";
