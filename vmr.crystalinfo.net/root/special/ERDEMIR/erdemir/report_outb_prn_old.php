   ...
   $write_to_csv = "";
    ...
    if($SUMM == "trunk" && $TRUNK!=""){
         $sql_str = "   SELECT CDR_MAIN_DATA.CDR_ID,LTRIM(CDR_MAIN_DATA.ORIG_DN) AS ORIG_DN,
                        DATE_FORMAT(MY_DATE,\"%m/%d\") AS TXTMY_DATE, DATE_FORMAT(MY_DATE,\"%d.%m.%Y\") AS MY_DATE, 
                        DATE_FORMAT(TIME_STAMP,\"%H:%i\") AS TXTMY_TIME,  DATE_FORMAT(TIME_STAMP,\"%H:%i:%s\") AS MY_TIME,  CDR_MAIN_DATA.COUNTER, 
         ...
         $sql_str  = "SELECT CDR_MAIN_DATA.CDR_ID,LTRIM(CDR_MAIN_DATA.ORIG_DN) AS ORIG_DN,DATE_FORMAT(CDR_MAIN_DATA.MY_DATE,\"%d/%m\") AS TXTMY_DATE,
                            ...
                            CDR_MAIN_DATA.DURATION, EXTENTIONS.DEPT_ID,CDR_MAIN_DATA.Locationid,  DATE_FORMAT(TIME_STAMP,\"%H:%i:%s\") AS MY_TIME,
                            CDR_MAIN_DATA.CountryCode, CDR_MAIN_DATA.LocalCode, CDR_MAIN_DATA.PURE_NUMBER, CDR_MAIN_DATA.COUNTER, 
                            CDR_MAIN_DATA.AUTH_ID, EXTENTIONS.DESCRIPTION, EXTENTIONS.ACCOUNT_NO, (CDR_MAIN_DATA.PRICE*$prc_fct) AS PRICE,
            ...
            if (mysql_num_rows($result)>0)
                ...
                while($row_array[$cnt] = mysql_fetch_array($result)){$cnt++;}
                    ...
                    if(is_array($row)) {
                        ...
                        if($CSV_EXPORT<>1 && $rep_type==1){
                            //Eðer Sadece þifreli çýkýþlar istenirse auth_code_description
                            if($AUTH_CODE_CNTL==1){
                                $name = substr($arr_auth_code[$row["AUTH_ID"]][0],0,18)."(".$row["ORIG_DN"].")";
                            }elseif($AUTH_CODE_CNTL==2){
                            //Eðer sadece þifresiz çýkýþlar istenirse orig_dn ve description
                                $name = $row["ORIG_DN"]." - ".substr($row["DESCRIPTION"],0,18);
                            }else{
                            // Eðer tümü istenirse auth kod varsa auth_code_description yoksa orig_dn ve description
                              if($row["AUTH_ID"]==""){
                                $name = $row["ORIG_DN"]." - ".substr($row["DESCRIPTION"],0,18);
                              }else{
                                $name = substr($arr_auth_code[$row["AUTH_ID"]][0],0,18)."(".$row["ORIG_DN"].")";
