...
//echo $sql_str.$sql_str1.$sql_str2;exit;
      ...
      $kriter  = ""; //Gelen çağrılar için kriterleri temizle
      //Temel kriterler. Verinini hızlı gelmesi için başa konuldu.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.SITE_ID"     ,  "=",  "$SITE_ID"); //Bu mutlaka olmalı.İlgili siteyi belirliyor.
      $kriter .= $cdb->field_query($kriter,   "ERR_CODE"     ,  "=",  "0"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.        
      ...
      $kriter .= $cdb->field_query($kriter,   "DURATION"     ,  "<",  "$max_acc_dur"); //Bu mutlaka olmalı.Dış arama olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ORIG_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ORIG_DN"     ,  "=",  "'$ORIG_DN'"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
...
//echo $sql_str.$sql_str1.$sql_str2;exit;
      ...
      $kriter  = ""; //Dahili çağrılar için kriterleri temizle
      //Temel kriterler. Verinini hızlı gelmesi için başa konuldu.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.SITE_ID"     ,  "=",  "$SITE_ID"); //Bu mutlaka olmalı.İlgili siteyi belirliyor.
      $kriter .= $cdb->field_query($kriter,   "ERR_CODE"     ,  "=",  "0"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.        
      $kriter .= $cdb->field_query($kriter,   "CALL_TYPE"     ,  "=",  "0"); //Bu mutlaka olmalı.Dış arama olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "DURATION"     ,  "<",  "$max_acc_dur"); //Bu mutlaka olmalı.Dış arama olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ORIG_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
...
//     }
     $sql_localout = $sql_loc1." GROUP BY ORIG_DN, TER_DN ";
      ...
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ORIG_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.TER_DN"     ,  "<>",  "''"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
      $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.TER_DN"     ,  "=",  "'$ORIG_DN'"); //Bu mutlaka olmalı.Hatasız kayıt olduğunu gösteriyor.
        ...
        <td width="100%" align="center" class="rep_header" align="center">
           <?if($CSV_EXPORT != 2){?>
            ...
            <TR>
                ...
                <TD width="50%" align=center CLASS="header"><?echo $company;?><BR>GİDEN-GELEN ÇAĞRI ANALİZİ</TD>
    ...
    <tr>
        <td>
            <table width="100%" border="0" bgcolor="C0C0C0" cellspacing="1" cellpadding="0" >
                <tr>
                <td colspan=4><b>Giden Çağrılar</b></td>
                </tr>
                <tr>
      ...
      ///////////////////////////////////////////////////////////////77
            ...
            if (mysqli_num_rows($result)>0)
                ...
                while($row_array[$cnt] = mysqli_fetch_array($result)){$cnt++;}
                    ...
                    foreach($row_array as $row){
                        ...
                        $csv_data[0] = $row["ORIG_DN"]." - ".substr($row["DESCRIPTION"],0,18); // Dahili ve Adı
                    ...
                    if(is_array($row)) {
                        ...
                        if ($TEL_ARRAY[0] == $local_country_code){
                            if ($TEL_ARRAY[1] == $LOC_CODE || $TEL_ARRAY[1]==""){//Şehiriçi Kabul edilen kod.
                ...
                $csv_data[0] = "";
                $csv_data[1] = "";
                $csv_data[2] = "Toplam Görüşme Adedi";
            ...
            ?>
                ...
                <tr>
                <td colspan=4><b>Giden Dahili Çağrılar</b></td>
                </tr>
                <tr>
                    <td class="rep_table_header" width="30%" ALIGN="center">Dahili</td>
...
<?
      ...
      unset($row_array);
            ...
            if (mysqli_num_rows($resultout)>0)
                ...
                while($row_array[$cnt] = mysqli_fetch_array($resultout)){$cnt++;}
...
?>
                ...
                <tr>
                <td colspan=4><b>Gelen Çağrılar</b></td>
                </tr>
                <tr>
                    <td class="rep_table_header" width="30%" ALIGN="center">Dahili</td>
      ...
      ///////////////////////////////////////////////////////////////77
            ...
            ?>
                ...
                <tr>
                <td colspan=4><b>Gelen Dahili Çağrılar</b></td>
                </tr>
                <tr>
                    <td class="rep_table_header" width="30%" ALIGN="center">Dahili</td>
            ...
            if (mysqli_num_rows($resultout)>0)
                ...
                while($row_array[$cnt] = mysqli_fetch_array($resultout)){$cnt++;}
                    ...
                    foreach($row_array as $row){
                        ...
                        $csv_data[0] = $row["TER_DN"]." - ".substr($row["DESCRIPTION"],0,18); // Dahili ve Adı
