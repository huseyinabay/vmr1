<?php
   ...
   require_valid_login();
   $SITE_ID = 1;
   if($p==""){$p='price';}//p gelmemişse ücret olsun
   ...
   if ($fld==""){exit;}//fld hala atanmamışsa devam etmemeli
   ...
   if (is_array($res_arr)){
     ...
     if ($res_arr[$i][0][1]==$SITE_ID){ //İlgili kaydın ilk alanının değeri
       if ($res_arr[$i][1][1] < 4) {
         $data[] = $res_arr[$i][$fld][1]; // Şehir içi Süre/Tutar/Adet
...
//      $titleObj = $c->addTitle(turkish2utf("Outbound Dağılımlar"), "verdana.ttf", 10);
