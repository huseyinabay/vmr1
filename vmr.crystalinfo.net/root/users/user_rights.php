...
?><body bgcolor="F7FBFF"><?
   ...
   if ($ekle && $id && $eklenen)
   {
      mysqli_query($conn ,"INSERT INTO USERS_RIGHTS (USER_ID, RIGHT_ID) VALUES ('$id', '$eklenen')");
   }
   // CIKARMA <<
   if ($cikar && $id && $cikarilan)
   {
      mysqli_query($conn, "DELETE FROM USERS_RIGHTS WHERE RIGHT_ID = $cikarilan AND USER_ID = $id");
   }
...
<form name="user_transfer" method="post" action="user_rights.php?act=<? echo $_GET['act'] ?>">
...
<select name=eklenen size="6" style="min-width:120px" class="select1">
         <?
...
<td width=180 valign=top>
<select name=cikarilan size="6" style="min-width:120px" class="select1">
         <?
